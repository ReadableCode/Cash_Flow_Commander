...
Real, understood problems in this repo that are not being fixed right now. Each
entry carries the evidence, the exact fix, and what the fix disturbs, so it can
be picked up without re-deriving the diagnosis. Re-run an entry's `verify:` line
before acting on it — reality moves.
